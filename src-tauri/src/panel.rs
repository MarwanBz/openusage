// ── macOS implementation (uses NSPanel for floating‐panel behaviour) ────────
#[cfg(target_os = "macos")]
mod macos {
    ...
    macro_rules! get_or_init_panel {
        ($app_handle:expr) => {
            match $app_handle.get_webview_panel("main") {
                Ok(panel) => Some(panel),
                Err(_) => {
                    if let Err(err) = crate::panel::macos::init($app_handle) {
...
// ── Linux implementation (uses standard tauri window APIs) ──────────────────
#[cfg(target_os = "linux")]
mod linux {
    use tauri::{AppHandle, Manager, Position, Size};
    ...
    /// Initialize the window (no-op on Linux; the window is created by Tauri config).
    pub fn init(_app_handle: &tauri::AppHandle) -> tauri::Result<()> {
        Ok(())
    }

    /// Position the window near the tray icon.
    /// On Linux the tray-click rect position is used directly.
    pub fn position_panel_at_tray_icon(
        app_handle: &tauri::AppHandle,
        icon_position: Position,
        icon_size: Size,
        ...
        let (_icon_w, icon_h) = match &icon_size {
            Size::Physical(s) => (s.width as f64, s.height as f64),
            Size::Logical(s) => (s.width, s.height),
        };

        // Read panel width from config
        let panel_width = match (window.outer_size(), window.scale_factor()) {
            (Ok(s), Ok(win_scale)) => s.width as f64 / win_scale,
            _ => {
                let conf: serde_json::Value =
